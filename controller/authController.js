...
if(!token) {
 ...
 const decoded = await promisify(jwt.verify)(token, process.env.JWT_TOKEN_SECRET)
  ...
  if(!currentUser) {
     return next(new AppError('The user is belonging to this token does no longer exist', 401))
  }
...
export const forgotpassword = catchAsync(async (req, res, next) => {
    ...
    const resetURL = `${req.protocol}://${req.get('http')}/api/v1/users/resetPassword/${resetToken}`
...
export const resetPassword = catchAsync(async (req, res, next) => {
    ...
    const hasherToken = crypto.createHash('sha256').update(req.params.token).digest('hex');

    const user = await User.findOne( { passwordResetToken: hasherTokenToken, passwordResetExpires:{ $gt: Date.now()} })
